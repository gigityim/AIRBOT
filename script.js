...
function initLoginModal() {
    ...
    document.addEventListener("keydown", (e) => { if (e.key === "Escape" && modal.classList.contains("is-open")) closeLoginModal(); });
...
function initMobileMenu() {
    ...
    if(mobileMenuBtn) {
        ...
        mobileMenuBtn.addEventListener("click", () => {
            ...
            mobileMenuBtn.setAttribute("aria-label", isOpen ? "打开菜单" : "关闭菜单");
        });
    }
...
function initPointerMotion() {
    ...
    window.addEventListener('pointermove', (event) => {
        ...
        tiltTarget.style.transform = `perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg)`;
    }, { passive: true });

    document.addEventListener('pointerover', (event) => {
        if (event.target.closest('a, button, input, textarea, [data-tilt]')) cursor.classList.add('is-hover');
    });

    document.addEventListener('pointerout', (event) => {
        if (event.target.closest('a, button, input, textarea, [data-tilt]')) cursor.classList.remove('is-hover');
        const tiltTarget = event.target.closest('[data-tilt]');
        if (tiltTarget && !tiltTarget.contains(event.relatedTarget)) tiltTarget.style.transform = '';
    });

    renderCursor();
}

function initVisualAtlas() {
    const cards = document.querySelectorAll('.atlas-card');
    if (!cards.length) return;

    cards.forEach((card) => {
        const trigger = card.querySelector('.atlas-toggle');
        ...
        trigger.addEventListener('click', () => {
            ...
            caption.textContent = title ? title.textContent : sourceImage.alt;
            closeButton.type = 'button';
            closeButton.textContent = '×';
            closeButton.setAttribute('aria-label', '关闭大图');

            figure.append(image, caption);
            viewer.append(figure, closeButton);
